...
public class SeparableEnemySolver {
  ...
  public boolean isSeparable() {
    Iterator<String> attendees = g.labels().iterator();
    HashSet<String> group1 = new HashSet<>();
    HashSet<String> group2 = new HashSet<>();

    while (attendees.hasNext()) {
      String next = attendees.next();
      Set<String> enemies = g.neighbors(next);
      Set<String> enemiesInGroupOne = new HashSet<String>(enemies);
      enemiesInGroupOne.retainAll(group1);
      Set<String> enemiesInGroupTwo = new HashSet<String>(enemies);
      enemiesInGroupTwo.retainAll(group2);
      if (enemiesInGroupOne.isEmpty()) {
        group1.add(next);
        group2.addAll(enemies);
      } else {
        if (enemiesInGroupTwo.isEmpty()) {
          group2.add(next);
          group1.addAll(enemies);
        } else {
          return false;
        }
      }
  ...
  /**
   * Reads an entire CSV and returns a List of Lists. Each inner List represents a
   * line of the CSV with each comma-seperated value as an entry. Assumes CSV file
   * does not contain commas except as separators. Returns null if invalid
   * filename.
   *
